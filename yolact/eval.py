...
def prep_display(dets_out, img, path, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    """
    ...
    for j in range(num_dets_to_consider):
        if scores[j] < args.score_threshold:
            ...
            break
    df = pd.DataFrame(columns=['path', 'class', 'score', 'x1', 'y1', 'x2', 'y2'])
    c = 1
    for i in reversed(range(num_dets_to_consider)):
        ...
        y2 = boxes[i, 3]
        score = scores[i]
        classname = cfg.dataset.class_names[classes[i]]
        # print(path, classname, score, xbox)
        df.loc[c] = [path, classname, score, x1, y1, x2, y2]
        c += 1
        # df.append({'path':path, 'class':classname, 'score':score, 'box':xbox}, ignore_index=True)
    # print(df)
    df.to_csv('final_info/info_' + str(path).split('/')[-1] + '.csv')  
    ...
    if num_dets_to_consider == 0:
        return classes, scores, areas, img_numpy
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    classes, scores, areas, img_numpy = prep_display(preds, frame, path, None, None, undo_transform=False)
...
def evaluate(net:Yolact, dataset, train_mode=False):
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            if args.display:
                classes, scores, areas, img_numpy = prep_display(preds, img, h, w)
            elif args.benchmark:
